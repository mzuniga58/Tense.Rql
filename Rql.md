...
<p>The arguments to an operation can be either a <i>PROPERTY</i> or a <i>VALUE</i>. Certain operators, primarily the filter operations, take only two agruments, one <i>PROPERTY</i> and one <i>VALUE</i>. The Equality operator is one such operator. The equality operator takes the value of the <i>PROPERTY</i> and checks to see if it is equal to a constant <i>VALUE</i>. 
